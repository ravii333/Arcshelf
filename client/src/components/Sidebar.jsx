...
function Sidebar({ isOpen, setIsOpen }) {
  ...
  return (
    <>
      ...
      >
        <div className="p-6 border-b border-gray-200/50 h-20 flex items-center justify-between bg-gradient-to-r from-blue-50 to-purple-50">
          <Link to="/" onClick={() => setIsOpen(false)} className="flex items-center space-x-3">
            <div className="w-10 h-10 bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl flex items-center justify-center">
              <span className="text-white font-bold text-lg">A</span>
            </div>
            <span className="text-2xl font-bold gradient-text">ArcShelf</span>
          </Link>
          <button onClick={() => setIsOpen(false)} className="p-2 rounded-xl text-gray-500 hover:bg-gray-100 hover:text-gray-700 transition-all duration-200" aria-label="Close sidebar">
          ...
          <div className="mb-8">
            ...
            <div className="space-y-2">
              {Object.keys(navStructure).map((uni) => (
                <div key={uni} className="bg-gray-50 rounded-xl overflow-hidden">
                  <button 
                    onClick={() => toggleUniversity(uni)}
                    className="w-full flex items-center justify-between px-4 py-3 text-left text-sm font-semibold text-gray-800 hover:bg-gray-100 transition-all duration-200"
                  >
                    ...
                    <ChevronRightIcon 
                      className={`h-5 w-5 text-gray-400 transition-transform duration-200 ${
                        openUniversities[uni] ? "rotate-90" : ""
                      }`} 
                    />
                  </button>
                  <div 
                    ...
                    <div className="px-4 py-2 space-y-1 bg-white/50">
                      {Object.keys(navStructure[uni]).map((college) => (
                        <div key={college} className="py-2">
                          <div className="px-3 py-2 font-medium text-gray-700 text-sm bg-blue-50 rounded-lg mb-2">
                            {college}
                          </div>
                          <div className="pl-4 space-y-1">
                            {Array.from(navStructure[uni][college]).map((course) => (
                              <Link 
                                ...
                                to="#" 
                                onClick={() => setIsOpen(false)} 
                                className="block px-3 py-2 text-sm text-gray-600 rounded-lg hover:bg-blue-100 hover:text-blue-700 transition-all duration-200"
                              >
                                {course}
                              </Link>
                            ))}
